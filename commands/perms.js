        ...
        if (subCommand === 'add') {
            ...
            .then((response) => {
                ...
                if(message === "Success") { 
                    ...
                    interaction.reply({embeds: [embed], ephemeral: true})
                } else if(message === "Maks 3 users as a normal user") { 
                    embed = new MessageEmbed()
                        .setColor('#5b8abf')
                        .setTitle("Permission")
                        .setDescription(`Du kan kun tilføje permission til 3 andre spillere!`)
                    interaction.reply({embeds: [embed], ephemeral: true})
                } else if(message === "Maks 10 users with Booster" || message === "Maks 10 users with VIP") { 
                    embed = new MessageEmbed()
                        .setColor('#5b8abf')
                        .setTitle("Permission")
                        .setDescription(`Du kan kun tilføje permission til 10 andre spillere!`)
                    interaction.reply({embeds: [embed], ephemeral: true})
                } else if(message === "Maks 20 users with Booster and VIP") { 
                    embed = new MessageEmbed()
                        .setColor('#5b8abf')
                        .setTitle("Permission")
